...
export const astroAsseetsOptimizer: ImagesOptimizer = async (
  ...
  return Promise.all(
    breakpoints.map(async (w: number) => {
      const h = _width && _height ? Math.round((w / _width) * _height) : undefined;
      try {
        const result = await getImage({
          src: image,
          width: w,
          ...(h !== undefined ? { height: h } : { inferSize: true }),
        ...
        const fallbackSrc = typeof image === 'string' ? image : image.src;
        return { src: fallbackSrc, width: w, height: h };
      }
    })
  );
};
...
export async function getImagesOptimized(
  image: ImageMetadata | string,
  {
    ...
    layout = 'constrained',
    style = '',
    format = config.formats[0].replace('image/', ''),
  ...
  const fallbackSrc = transformedImages.find((candidate) => candidate.width >= (width || 0))?.src ?? transformedImages.at(-1)?.src;
