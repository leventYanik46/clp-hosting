---
metadata:
  title: Direito de Lesões Pessoais
  description: >-
    Nossa equipe de Lesões Pessoais protege clientes prejudicados por
    negligência, ajudando-os a buscar justiça, compensação e tranquilidade em
    momentos difíceis.
hero:
  tagline: Fale Conosco
  title: Vamos conversar!
  image:
    src: ~/assets/images/high-resolution-photo-of-us-capitol-l.jpeg
    alt: Us capitol building image
contactForm:
  title: Envie uma mensagem hoje mesmo!
  subtitle: >-
    Receba orientação jurídica especializada adaptada às suas necessidades. Fale
    conosco para começar!
  infoTitle: Direito de Lesões Pessoais
  infoDescription: >-
    Nossa equipe de Lesões Pessoais protege clientes prejudicados por
    negligência, ajudando-os a buscar justiça, compensação e tranquilidade em
    momentos difíceis.
  googleReview:
    label: Avaliações do Google
    rating: 5
    count: 97
    url: >-
      https://www.google.com/maps/place/Capitol+Law+Partners/data=!4m2!3m1!1s0x0:0xdd25a8b02491aacd?sa=X&ved=1t:2428&ictx=111
  inputs:
    name: Nome
    email: E-mail
    phone: Telefone
  textarea: Mensagem
  disclaimer: Ao enviar este formulário, você concorda com a coleta de dados.
  description: Nossa equipe normalmente responde em até 24 horas úteis.
post:
  title: Direito de Lesões Pessoais
  image: ~/assets/images/personal-injury.jpg
  content: >
    <ul>
      <li><a href="/pt/practice-area/key-services/car-truck-and-motorcycle-accidents">Acidentes de carro, caminhão e motocicleta</a></li>
      <li><a href="/pt/practice-area/key-services/pedestrian-and-bicycle-accidents">Acidentes com pedestres e bicicletas</a></li>
      <li><a href="/pt/practice-area/key-services/medical-negligence">Negligência médica</a></li>
      <li><a href="/pt/practice-area/key-services/dangerous-or-defective-products">Produtos perigosos ou defeituosos</a></li>
      <li><a href="/pt/practice-area/key-services/wrongful-death">Morte por negligência</a></li>
    </ul>


    <p>Se você se feriu por causa da negligência de outra pessoa, não precisa
    enfrentar as seguradoras sozinho. Na Capitol Law Partners, defendemos
    indivíduos e famílias nos momentos mais difíceis de suas vidas.</p>


    <h3>O Que Esperar da Nossa Equipe</h3>


    <p><strong>Orientação clara — sem juridiquês</strong><br />

    Explicamos seus direitos de forma clara para que você sempre saiba como está
    o processo.</p>


    <p><strong>Nós lidamos com as seguradoras — você foca na
    recuperação</strong><br />

    Da coleta de provas à negociação do acordo, cuidamos da pressão jurídica e
    financeira para você.</p>


    <p><strong>Sem honorários advocatícios se não ganharmos</strong><br />

    Atendemos a maioria dos casos de lesão pessoal em regime de contingência —
    você não paga nada adiantado.</p>


    <h3>Sua Recuperação Começa com uma Conversa</h3>


    <p>Seja sua lesão leve ou grave, ouvimos sua história e montamos uma
    estratégia personalizada. Se não tiver certeza se tem um caso, pergunte — a
    consulta é gratuita e confidencial.</p>


    <p>Deixe-nos lutar pela compensação e tranquilidade que você merece.</p>
seo:
  title: Direito de Lesões Pessoais
  description: >-
    Nossa equipe de Lesões Pessoais protege clientes prejudicados por
    negligência, ajudando-os a buscar justiça, compensação e tranquilidade em
    momentos difíceis.
  schema:
    mode: merge
    includeBreadcrumbs: true
    entity: service
---
