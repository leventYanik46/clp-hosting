---
metadata:
  title: Business & Employment Immigration Solutions
  description: Workforce immigration strategies that move talent compliantly and on time.
hero:
  tagline: Contact Us
  title: Let's Connect!
  image:
    src: ~/assets/images/high-resolution-photo-of-us-capitol-l.jpeg
    alt: Us capitol building image
contactForm:
  title: Drop us a message today!
  subtitle: >-
    Get expert legal guidance tailored to your needs. Contact us now to start
    your journey!
  inputs:
    name: Name
    email: Email
    phone: Phone Number
  textarea: Message
  disclaimer: By submitting this form, you agree to collection of data.
  description: Our team typically responds within 24 business hours.
  googleReview:
    label: Google Reviews
    rating: 5
    count: 97
    url: >-
      https://www.google.com/maps/place/Capitol+Law+Partners/data=!4m2!3m1!1s0x0:0xdd25a8b02491aacd?sa=X&ved=1t:2428&ictx=111
  infoTitle: Business & Employment Immigration Solutions
  infoDescription: Workforce immigration strategies that move talent compliantly and on time.
post:
  title: Business & Employment Immigration Solutions
  image: ~/assets/images/business-immigration-img.jpg
  content: >

    <p>Workforce immigration strategies that move talent compliantly and on
    time.</p>


    <h3>Key Services</h3>

    <ul>
          <li>Nonimmigrant visas (H-1B, L-1, E, TN, O-1)</li>
          <li>PERM, I-140, and employment-based green cards</li>
          <li>Corporate policies, I-9, and compliance planning</li>
          <li>Mobility planning for startups and multinational teams</li>
        </ul>

    <h3>Why It Matters</h3>

    <p>We align strategy, documents, and execution so your goals stay protected.
    Every matter is handled with clear communication, pragmatic advice, and a
    focus on speed and risk reduction.</p>
seo:
  title: Business & Employment Immigration Solutions
  description: Workforce immigration strategies that move talent compliantly and on time.
  schema:
    mode: merge
    includeBreadcrumbs: true
    entity: service
---
