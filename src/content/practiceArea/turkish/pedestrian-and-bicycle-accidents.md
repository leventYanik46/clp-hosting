---
metadata:
  title: Yaya ve Bisiklet Kazaları
  description: >-
    Yaralanan yayaları ve bisikletlileri hızlı hareketle delil ve tazminat
    güvenceye alarak korumak.
hero:
  tagline: Bize Ulaşın
  title: Bağlanalım!
  image:
    src: ~/assets/images/high-resolution-photo-of-us-capitol-l.jpeg
    alt: ABD Kongre Binası görseli
contactForm:
  title: Hemen bize mesaj gönderin!
  subtitle: İhtiyacınıza özel hukuki rehberlik için bizimle iletişime geçin.
  inputs:
    name: Ad
    email: E-posta
    phone: Telefon Numarası
  textarea: Mesaj
  disclaimer: Bu formu göndererek verilerinizin toplanmasını kabul edersiniz.
  description: Ekibimiz genellikle 24 iş saati içinde yanıt verir.
  googleReview:
    label: Google Yorumları
    rating: 5
    count: 97
    url: >-
      https://www.google.com/maps/place/Capitol+Law+Partners/data=!4m2!3m1!1s0x0:0xdd25a8b02491aacd?sa=X&ved=1t:2428&ictx=111
  infoTitle: Yaya ve Bisiklet Kazaları
  infoDescription: >-
    Yaralanan yayaları ve bisikletlileri hızlı hareketle delil ve tazminat
    güvenceye alarak korumak.
post:
  title: Yaya ve Bisiklet Kazaları
  image: ~/assets/images/personal-injury.jpg
  content: >

    <p>Yaralanan yayaları ve bisikletlileri hızlı hareketle delil ve tazminat
    güvenceye alarak korumak.</p>


    <h3>Temel Hizmetler</h3>

    <ul>
          <li>Sorumluluk analizi ve kaza yeniden oluşturma</li>
          <li>Sürücülere, belediyelere ve sigortacılara karşı talepler</li>
          <li>Tıbbi ve gelir kaybı dokümantasyonu</li>
          <li>Ciddi yaralanma davaları için dava stratejisi</li>
        </ul>

    <h3>Neden Önemli?</h3>

    <p>Strateji, belgeler ve uygulamayı uyumlu hale getirerek hedeflerinizin
    korunmasını sağlıyoruz. Her konu, açık iletişim, pragmatik tavsiye ve hız
    ile risk azaltmaya odaklanılarak ele alınır.</p>
seo:
  title: Yaya ve Bisiklet Kazaları
  description: >-
    Yaralanan yayaları ve bisikletlileri hızlı hareketle delil ve tazminat
    güvenceye alarak korumak.
  schema:
    mode: merge
    includeBreadcrumbs: true
    entity: service
---
