---
metadata:
  title: Rencontrez notre équipe
  description: Rencontrez notre équipe et prenez rendez-vous avec nos experts.
sections:
  - wrapper: flex flex-wrap justify-center gap-4
    members:
      - title: Cagatay Ersoy
        tagline: AvocaCofondateur et associé gérantt
        subtitle: Travaillons ensemble
        bottomtag: Immigration d’affaires | Droit des affaires | Droit des marques
        actions:
          - variant: primary
            text: Prendre rendez-vous
            href: https://calendar.app.google/UGuCPvGbEV1b3acWA
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/cagatay
            target: _blank
        image:
          src: ~/assets/images/cagatay-bio-new-team.jpeg
          alt: Prendre rendez-vous
      - title: Sena Sahin
        tagline: Cofondatrice et associée gérante
        subtitle: Travaillons ensemble
        bottomtag: Contentieux | Droit des sociétés | Droit de l’immigration
        actions:
          - variant: primary
            text: Prendre rendez-vous
            href: https://calendar.app.google/j5WDBFZPNocPCB7CA
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/sena
            target: _blank
        image:
          src: ~/assets/images/sena-bio-new-team.jpeg
          alt: Prendre rendez-vous
      - title: Shafeek Seddiq
        tagline: Avocat Conseil
        subtitle: Travaillons ensemble
        bottomtag: Trusts et successions | Droit des affaires | Conseil juridique externe
        actions:
          - variant: primary
            text: Me contacter
            href: /fr/contact
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/shafeek
            target: _blank
        image:
          src: ~/assets/images/shafeek-bio-new-team.jpeg
          alt: Image illustrative
      - title: Esra Nur Bulan
        tagline: Avocate
        subtitle: Travaillons ensemble
        bottomtag: Contentieux de l’immigration | Immigration
        actions:
          - variant: primary
            text: Me contacter
            href: https://wa.me/12024402272
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/esra
            target: _blank
        image:
          src: ~/assets/images/esra-bio-new-team.jpeg
          alt: Image illustrative
      - title: Mehmet Y. Turkoglu
        tagline: Avocat
        subtitle: Travaillons ensemble
        bottomtag: >-
          Asile | Regroupement familial | Naturalisation | Immigration basée sur
          l'emploi
        actions:
          - variant: primary
            text: Me contacter
            href: /fr/contact
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/mehmet
            target: _blank
        image:
          src: ~/assets/images/mehmet-bio-new-team.jpeg
          alt: Mehmet Y. Turkoglu
      - title: S. Selman Kilic
        tagline: Avocat
        subtitle: Travaillons ensemble
        bottomtag: Contentieux | Droit des sociétés | Droit de l’immigration
        actions:
          - variant: primary
            text: Me contacter
            href: https://wa.me/12024402272
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/selman
            target: _blank
        image:
          src: ~/assets/images/selman-bio-new-team.jpeg
          alt: S. Selman Kilic
      - title: Omar Bississo
        tagline: Avocat en défense pénale
        subtitle: Travaillons ensemble
        bottomtag: Défense pénale | Contentieux civil
        actions:
          - variant: primary
            text: Me contacter
            href: /fr/contact
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/omar
            target: _blank
        image:
          src: ~/assets/images/omar-bio-new-tean.jpeg
          alt: Omar Bississo
      - title: Kemal Sahin
        tagline: Assistant juridique
        subtitle: Travaillons ensemble
        bottomtag: Conseil juridique général | Propriété intellectuelle
        actions:
          - variant: primary
            text: Me contacter
            href: https://wa.me/12024402272
            target: _blank
          - variant: primary
            text: Voir la bio →
            href: /fr/our-team/kemal
            target: _blank
        image:
          src: ~/assets/images/kemal.jpeg
          alt: Kemal Sahin
  - wrapper: w-full
    members:
      - title: null
        subtitle: Dr. Murat Y.
        tagline: Office Manager
        actions:
          - variant: primary
            text: Me contacter
            href: https://wa.me/12024402272
            target: _blank
  - wrapper: w-full grid grid-cols-1 md:grid-cols-2 gap-4
    members:
      - title: null
        subtitle: Dr. Burak C.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Mikayil G.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Dr. Ahmet B.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Yasin D.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Rana B.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Ebrar T.
        tagline: Responsable de dossiers
      - title: null
        subtitle: Said M.
        tagline: Directeur marketing
  - wrapper: w-full
    members:
      - title: null
        subtitle: Kilare M.
        tagline: Assistant juridique
        actions:
          - variant: primary
            text: Me contacter
            href: https://wa.me/12024402272
            target: _blank
seo:
  title: Rencontrez notre équipe
  description: Rencontrez notre équipe et prenez rendez-vous avec nos experts.
---
